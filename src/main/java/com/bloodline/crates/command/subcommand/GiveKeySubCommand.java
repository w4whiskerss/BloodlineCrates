...
public class GiveKeySubCommand implements SubCommand {
    ...
    public GiveKeySubCommand(BloodlineCrates plugin) {
        this.plugin = plugin;
    }
    ...
    public String getDescription() {
        return "Give crate keys to a player based on the crate key mode";
    }
    ...
    public String getPermission() {
        return "bloodcrates.admin.givekey";
    }

    @Override
    public List<String> getAliases() {
        return Collections.singletonList("gk");
    }

    @Override
    public void execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(plugin.getConfigManager().getPrefix() + "§cUsage: /bc givekey <crate_id> <player> [amount]");
            return;
        }
        ...
        if (args.length >= 3) {
            try {
                ...
                if (amount < 1 || amount > 64) {
                    sender.sendMessage(plugin.getConfigManager().getPrefix() + "§cAmount must be between 1 and 64");
                    return;
                }
        ...
        if (crate.getKeyMode() == KeyMode.VIRTUAL) {
            ...
            plugin.getKeyManager().giveKey(target, crateKey, amount);
        }

        if (plugin.getSyncManager() != null) {
            plugin.getSyncManager().publishKeyGrant(target.getUniqueId(), crate.getId(), amount);
        }
        plugin.getAuditLogger().log(new AuditEntry(
            ...
            target.getName(),
            crate.getId() + " " + crate.getKeyMode().name().toLowerCase() + " x" + amount,
            System.currentTimeMillis()
        ));

        String modeLabel = crate.getKeyMode() == KeyMode.VIRTUAL ? "virtual " : "physical ";
        sender.sendMessage(plugin.getConfigManager().getPrefix() + "§aGave " + amount + "x " + modeLabel + crate.getDisplayName() + " §6Key §ato " + target.getName());
        target.sendMessage(plugin.getConfigManager().getPrefix() + "§aYou received " + amount + "x " + modeLabel + crate.getDisplayName() + " §6Key§a!");
    }
